...
def paramurl_view(request):
    ...
    if campaignname is not None:
        phone = Contact.objects.filter(phonenumber=phonenumber).first()

        if campaignname in dat:
            dat_campaign = Dat.objects.filter(Contact=phone).filter(campaignname=campaignname).first()
            # return HttpResponse('<h1>Dat numero {} {} {}</h1>'.format(Contact, dat_campaign.id, dat_campaign.Statut))
            context= {
        ...
        elif campaignname in acquisition:
            acquisition_campaign = Acquisition.objects.filter(
                Contact=phone).filter(campaignname=campaignname).first()
